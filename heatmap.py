...
class yolov12_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize, class_names=None):
        ...
        # ✅ 优先使用你传入的 class_names，否则退回到权重内置的 ckpt 名字
    ...
    def __call__(self, img_path, save_path, k=30, shuffle_seed=42):
        # remove dir if exist
        ...
        if os.path.isdir(img_path):
            ...
            files = [f for f in os.listdir(img_path) if os.path.splitext(f)[1].lower() in exts]
            if not files:
                print(f"[WARN] 目录下未发现图片文件：{img_path}")
                return
            ...
            for fname in files:
                ...
                out_path = os.path.join(save_path, fname)  # 与输入同名保存
                self.process(in_path, out_path)
        else:
            # 单张图片
...
def get_params():
    params = {
        'weight': '/root/yolov12-main/runs/detect/train59/weights/best.pt', # 只需要指定权重即可
        'device': 'cuda:0',
        ...
        'show_box': False,
        'renormalize': True,
        'class_names': ['Sewage_Outfall']  # 👈 替换成你自己的
    }
    return params
